...
- **Schema**: `jcgkzx_monitoer`
- **前缀**: `wcnr_`
- **SQL规范**: 遵循 Kingbase V8 兼容性
  - 用 `IS NOT NULL` 代替 `<> ''`
  - 表名带schema前缀: `"jcgkzx_monitoer"."wcnr_xxx"`
  - 区域分组走字典表 `stdata.b_dic_zzjgdm`

### 2.2 管控对象池（核心基准表）

```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_target_pool" (
...
COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_target_pool" IS '未成年人管控对象池';
```
...
```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_ryrl_gj" (
...
CREATE INDEX idx_wcnr_ryrl_zjhm ON "jcgkzx_monitoer"."wcnr_ryrl_gj"(zjhm);
CREATE INDEX idx_wcnr_ryrl_shot_time ON "jcgkzx_monitoer"."wcnr_ryrl_gj"(shot_time);
CREATE INDEX idx_wcnr_ryrl_device ON "jcgkzx_monitoer"."wcnr_ryrl_gj"(device_id);

COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_ryrl_gj" IS '未成年人人脸轨迹（从t_spy_ryrlgj_xx抽取，libname=全市未成年人）';
```

**抽取SQL：**
```sql
INSERT INTO "jcgkzx_monitoer"."wcnr_ryrl_gj"
...
```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_ly_checkin" (
    id            BIGSERIAL PRIMARY KEY,
    zjhm          VARCHAR(18),
    xm            VARCHAR(50),
    xb            VARCHAR(4),
...
CREATE INDEX idx_wcnr_ly_zjhm ON "jcgkzx_monitoer"."wcnr_ly_checkin"(zjhm);
CREATE INDEX idx_wcnr_ly_rzsj ON "jcgkzx_monitoer"."wcnr_ly_checkin"(rzsj);

COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_ly_checkin" IS '未成年人旅馆入住（从t_ly_checkin_gn_merge抽取）';
```

**抽取SQL：**
```sql
INSERT INTO "jcgkzx_monitoer"."wcnr_ly_checkin"
...
WHERE l.zjhm IN (SELECT zjhm FROM "jcgkzx_monitoer"."wcnr_target_pool")
...
```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_rk_zp" (
    zjhm          VARCHAR(18) PRIMARY KEY,
    xm            VARCHAR(50),
...
COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_rk_zp" IS '未成年人照片（从t_dsfb_rk_zpxx/t_ap_czrk_zp抽取）';
```

**抽取SQL：**
```sql
INSERT INTO "jcgkzx_monitoer"."wcnr_rk_zp" (zjhm, xm, zp, zp_source)
SELECT r.zjhm, r.xm, r.zp, 'dsfb'
FROM "ywdata"."t_dsfb_rk_zpxx" r
WHERE r.zjhm IN (SELECT zjhm FROM "jcgkzx_monitoer"."wcnr_target_pool")
...
```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_czrk" (
    zjhm          VARCHAR(18) PRIMARY KEY,
    xm            VARCHAR(50),
    xb            VARCHAR(4),
    mz            VARCHAR(20),
...
COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_czrk" IS '未成年人常住人口信息（从t_ap_czrk_jbxx抽取）';
```

#### (5) 风险评分结果表

```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_score" (
    zjhm              VARCHAR(18) PRIMARY KEY,
...
COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_score" IS '未成年人风险评分结果';
```

#### (6) 评分历史表

```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_score_history" (
    id            BIGSERIAL PRIMARY KEY,
    zjhm          VARCHAR(18),
    total_score   INTEGER,
...
CREATE INDEX idx_wcnr_score_hist_zjhm ON "jcgkzx_monitoer"."wcnr_score_history"(zjhm);

COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_score_history" IS '未成年人风险评分历史（趋势分析用）';
```

#### (7) 预警记录表

```sql
CREATE TABLE "jcgkzx_monitoer"."wcnr_alert" (
    id            BIGSERIAL PRIMARY KEY,
    zjhm          VARCHAR(18),
    xm            VARCHAR(50),
...
CREATE INDEX idx_wcnr_alert_time ON "jcgkzx_monitoer"."wcnr_alert"(trigger_time DESC);
CREATE INDEX idx_wcnr_alert_zjhm ON "jcgkzx_monitoer"."wcnr_alert"(zjhm);

COMMENT ON TABLE "jcgkzx_monitoer"."wcnr_alert" IS '未成年人预警记录';
```
...
```sql
-- 管控总人数
SELECT COUNT(*) FROM "jcgkzx_monitoer"."wcnr_target_pool";

-- 高风险人数
SELECT COUNT(*) FROM "jcgkzx_monitoer"."wcnr_score"
