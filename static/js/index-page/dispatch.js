    ...
    function dispatchStatusMeta(status) {
      var map = {
        ...
        need_phone: { label: '待补手机号', badge: 'bg-amber-100 text-amber-700 ring-amber-200' }
      };
    ...
    function renderDispatchAuth() {
      ...
      if (text) {
        var msg = auth.authenticated
          ? '认证账号：' + (auth.username || '--') + ' · 剩余有效期 ' + (auth.expires_in || 0) + ' 秒'
          : '当前尚未认证任务平台。';
        if (auth.last_error) {
    ...
    function renderDispatchQueue() {
      ...
      if (!DISPATCH_STATE.items.length) {
        box.innerHTML = '<div class="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 px-4 py-6 text-sm text-slate-500">当前暂无待下发对象。完成人脸识别后会自动流转到这里。</div>';
        return;
      }
            ...
            '<div class="flex items-start justify-between gap-3">' +
              '<label class="flex min-w-0 flex-1 items-start gap-3">' +
                ...
                '<div class="min-w-0 flex-1">' +
                  ...
                  '<div class="mt-1 text-xs leading-6 text-slate-500">来源：' + dispatchEscapeHtml(item.source_name || item.source_type || '--') + ' · 违法行为：' + dispatchEscapeHtml(item.illegal_type || '--') + '</div>' +
                  '<div class="mt-1 text-xs leading-6 text-slate-500">手机号：' + dispatchEscapeHtml(item.person_phone || '待补充') + ' · 相似度 ' + dispatchEscapeHtml(item.similarity_score || 0) + '</div>' +
                '</div>' +
              '</label>' +
    ...
    function restoreDispatchMinimalPayload() {
      ...
      fetch('/dispatch/preview', {
        ...
        .then(function (data) {
          if (!data.ok) {
            alert(data.error || '恢复精简 Payload 失败');
            return;
          }
          setDispatchPayloadEditor(data.items.length === 1 ? data.items[0] : data.items);
        })
        .catch(function () {
          alert('恢复精简 Payload 请求失败');
        });
    }
    ...
    function sendDispatchTasks() {
      ...
      fetch('/dispatch/send', {
        ...
        .then(function (data) {
          ...
          alert((data.mock_mode ? '模拟模式：' : '') + '任务下发完成，共处理 ' + (data.count || 0) + ' 条');
          refreshDispatchTab();
        })
        .catch(function () {
          alert('任务下发请求失败');
    ...
    function sendDispatchSms() {
      ...
      fetch('/dispatch/sms/send', {
        ...
        .then(function (data) {
          ...
          alert((data.mock_mode ? '模拟模式：' : '') + '短信处理完成，共处理 ' + ((data.items || []).length) + ' 条');
          refreshDispatchTab();
        })
        .catch(function () {
          alert('短信发送请求失败');
