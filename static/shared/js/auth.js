const AUTH_STORAGE_USERS = 'falcon_sentinel_users_v1';
const AUTH_STORAGE_SESSION = 'falcon_sentinel_session_v1';
...
function handleLogin(event) {
  ...
  if (!username || !password) {
    if (feedback) feedback.textContent = '请输入警号和密码。';
    return false;
  }
...
function handleRegister(event) {
  ...
  if (users.some(function (item) { return item.username === username; })) {
    if (feedback) feedback.textContent = '该警号已存在，请直接登录。';
    return false;
  }
